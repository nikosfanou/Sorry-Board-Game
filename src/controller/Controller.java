...
public void initialize() {
	...
	view=new View();
	Pawn redPawn1=new Pawn(Color.RED,72); //72 is the position of the red start square
	Pawn redPawn2=new Pawn(Color.RED,72);
	player1=new Player("Player 1",Color.RED,redPawn1,redPawn2);
	Pawn yellowPawn1=new Pawn(Color.YELLOW,73); //73 is the position of the yellow start square
	Pawn yellowPawn2=new Pawn(Color.YELLOW,73);
	player2=new Player("Player 2",Color.YELLOW,yellowPawn1,yellowPawn2);
	setListeners();
	int cp =new Random().nextInt(2); //tixaia epilogi paixti gia na arxisei
	if(cp==0) {
	...
	// gia tin allagi tou message sto info box
				try {
					view.updateInfoBox( view.getInfoBox().getText(0, 9)+"\n\nTurn:"+
		            currentPlayer.toString()+"Cards Left: "+cardsNBoard.getCardsLeft()+"\n");
				} catch (BadLocationException ee) {
				System.out.println("ERROR");
				}
				System.out.println("New card\n");
	//isws i seira na mi dinetai ston player1 alla tyxaia
}
...
public void fold() { 
	if(cardsNBoard.getLatestCard() instanceof SorryCard) {
        ...
        c.ableToMove(currentPlayer.getSecondPawn(), cardsNBoard)==false &&
       	c.ableToMove(currentPlayer.getFirstPawn(), 6, currentPlayer.getSecondPawn(), 1, cardsNBoard)==false &&
       	c.ableToMove(currentPlayer.getFirstPawn(), 5, currentPlayer.getSecondPawn(), 2, cardsNBoard)==false &&
       	c.ableToMove(currentPlayer.getFirstPawn(), 4, currentPlayer.getSecondPawn(), 3, cardsNBoard)==false &&
       	c.ableToMove(currentPlayer.getFirstPawn(), 3, currentPlayer.getSecondPawn(), 4, cardsNBoard)==false &&
       	c.ableToMove(currentPlayer.getFirstPawn(), 2, currentPlayer.getSecondPawn(), 5, cardsNBoard)==false &&
       	c.ableToMove(currentPlayer.getFirstPawn(), 1, currentPlayer.getSecondPawn(), 6, cardsNBoard)==false) {
...
/**
 * Gets the fold button, Cards button and pawns buttons
 * and adds the right listeners to them.
 * @pre
 * @post Adds listeners to the fold, cards and pawns 
 * buttons.
 */
 ...
 * @pre
 * @post Returns true if one of the players won,else 
 * false.
...
/**A class which implements MouseListener. We need it 
 * for the clicks on the fold button.
*/
class FoldListener implements MouseListener{
	...
	public void mouseClicked(MouseEvent e) {
		if((JButton)e.getSource()==view.getFoldButton()) {
			//tsekaroume an teleiwse to paixnidi
...
/**A class which implements MouseListener. We need it 
 * for the clicks on the packOfCards button.
*/
class CardsListener implements MouseListener{
	/**
	 * What will happen if we click on the packOfCards
	 * button.
	 * @pre There is at least one card which hasn't 
	 * been played.
	 * @post A new card is played and the currentCard
	 * button changes its image according to the new 
	 * card.
	 */
...
/**A class which implements MouseListener. We need it 
 * for the clicks on the pawn buttons.
*/
class PawnListener implements MouseListener{
	/**
	 * What will happen if we click on one of the pawns.
	 * @pre At least one of the current player's pawns 
	 * can be moved.
	 * @post Moves the pawn which we clicked on.
	 */
	
	public void mouseClicked(MouseEvent e) {
if((JButton)e.getSource()==view.getPlayer1Pawns()[0]) { //prwto kokkino
	...
	if(cardsNBoard.getLatestCard() instanceof SorryCard) {
		...
		if(c.IsPlayed()==false) {
			...
			String s = (String)JOptionPane.showInputDialog(
			                    ...
			                    "");
			if(s.equals(moves[0])) {
				if(c.ableToMove(currentPlayer.getFirstPawn(),player2.getFirstPawn(), cardsNBoard)==true) {
					c.movePawn(currentPlayer.getFirstPawn(),player2.getFirstPawn(), cardsNBoard);
					view.updatePawn(player1.getFirstPawn(), player1.getSecondPawn(), player2.getFirstPawn(), player2.getSecondPawn());
					c.DropCard();
					//tsekaroume an teleiwse to paixnidi
					if(checkIfGameFinished()) {
						showWinningMessage("The winner is "+currentPlayer.toString()+"\n");
               ...
               if(c.ableToMove(currentPlayer.getFirstPawn(),player2.getSecondPawn(), cardsNBoard)==true) {
		...
		NumberSevenCard c=(NumberSevenCard) cardsNBoard.getLatestCard();
		if(c.IsPlayed()==false) {
			String[] moves = {"Move pawn1 by 7", "Move pawn1 by 6 and pawn2 by 1", "Move pawn1 by 5 and pawn2 by 2", "Move pawn1 by 4 and pawn2 by 3"
					, "Move pawn1 by 3 and pawn2 by 4", "Move pawn1 by 2 and pawn2 by 5", "Move pawn1 by 1 and pawn2 by 6"};
			String s = (String)JOptionPane.showInputDialog(
			                    view,
			                    "SelectMove\n",
			                    "Customized Dialog",
			...
			if(s.equals(moves[0])) {
               ...
               if(c.ableToMove(currentPlayer.getFirstPawn(),6,currentPlayer.getSecondPawn(),1, cardsNBoard)==true) {
					c.movePawn(currentPlayer.getFirstPawn(),6,currentPlayer.getSecondPawn(),1, cardsNBoard);
					view.updatePawn(player1.getFirstPawn(), player1.getSecondPawn(), player2.getFirstPawn(), player2.getSecondPawn());
					c.DropCard();
					//tsekaroume an teleiwse to paixnidi
					if(checkIfGameFinished()) {
	               ...
	               if(c.ableToMove(currentPlayer.getFirstPawn(),5,currentPlayer.getSecondPawn(),2, cardsNBoard)==true) {
						c.movePawn(currentPlayer.getFirstPawn(),5,currentPlayer.getSecondPawn(),2, cardsNBoard);
						view.updatePawn(player1.getFirstPawn(), player1.getSecondPawn(), player2.getFirstPawn(), player2.getSecondPawn());
						c.DropCard();
						//tsekaroume an teleiwse to paixnidi
						if(checkIfGameFinished()) {
					 ...
					 if(c.ableToMove(currentPlayer.getFirstPawn(),4,currentPlayer.getSecondPawn(),3, cardsNBoard)==true) {
							c.movePawn(currentPlayer.getFirstPawn(),4,currentPlayer.getSecondPawn(),3, cardsNBoard);
							view.updatePawn(player1.getFirstPawn(), player1.getSecondPawn(), player2.getFirstPawn(), player2.getSecondPawn());
							c.DropCard();
							//tsekaroume an teleiwse to paixnidi
							if(checkIfGameFinished()) {
					...
					if(c.ableToMove(currentPlayer.getFirstPawn(),3,currentPlayer.getSecondPawn(),4, cardsNBoard)==true) {
						c.movePawn(currentPlayer.getFirstPawn(),3,currentPlayer.getSecondPawn(),4, cardsNBoard);
						view.updatePawn(player1.getFirstPawn(), player1.getSecondPawn(), player2.getFirstPawn(), player2.getSecondPawn());
						c.DropCard();
						//tsekaroume an teleiwse to paixnidi
						if(checkIfGameFinished()) {
					...
					if(c.ableToMove(currentPlayer.getFirstPawn(),2,currentPlayer.getSecondPawn(),5, cardsNBoard)==true) {
						c.movePawn(currentPlayer.getFirstPawn(),2,currentPlayer.getSecondPawn(),5, cardsNBoard);
						view.updatePawn(player1.getFirstPawn(), player1.getSecondPawn(), player2.getFirstPawn(), player2.getSecondPawn());
						c.DropCard();
						//tsekaroume an teleiwse to paixnidi
						if(checkIfGameFinished()) {
					...
					if(c.ableToMove(currentPlayer.getFirstPawn(),1,currentPlayer.getSecondPawn(),6, cardsNBoard)==true) {
						c.movePawn(currentPlayer.getFirstPawn(),1,currentPlayer.getSecondPawn(),6, cardsNBoard);
						view.updatePawn(player1.getFirstPawn(), player1.getSecondPawn(), player2.getFirstPawn(), player2.getSecondPawn());
						c.DropCard();
						//tsekaroume an teleiwse to paixnidi
						if(checkIfGameFinished()) {
		...
		}else if((JButton)e.getSource()==view.getPlayer1Pawns()[1]) { //2o kokkino
			if(currentPlayer.getColor()==Color.RED) {
				...
				if(cardsNBoard.getLatestCard() instanceof SorryCard) {
					...
					if(c.IsPlayed()==false) {
						...
						String s = (String)JOptionPane.showInputDialog(
						                    ...
						                    "");
						if(s.equals(moves[0])) {
							if(c.ableToMove(currentPlayer.getSecondPawn(),player2.getFirstPawn(), cardsNBoard)==true) {
			               ...
			               if(c.ableToMove(currentPlayer.getSecondPawn(),player2.getSecondPawn(), cardsNBoard)==true) {
					...
					NumberSevenCard c=(NumberSevenCard) cardsNBoard.getLatestCard();
					if(c.IsPlayed()==false) {
						String[] moves = {"Move pawn2 by 7", "Move pawn2 by 6 and pawn1 by 1", "Move pawn2 by 5 and pawn1 by 2", "Move pawn2 by 4 and pawn1 by 3"
								, "Move pawn2 by 3 and pawn1 by 4", "Move pawn2 by 2 and pawn1 by 5", "Move pawn2 by 1 and pawn1 by 6"};
						String s = (String)JOptionPane.showInputDialog(
						                    view,
						                    "SelectMove\n",
						                    "Customized Dialog",
						...
						if(s.equals(moves[0])) {
			               ...
			               if(c.ableToMove(currentPlayer.getSecondPawn(),6,currentPlayer.getFirstPawn(),1, cardsNBoard)==true) {
								c.movePawn(currentPlayer.getSecondPawn(),6,currentPlayer.getFirstPawn(),1, cardsNBoard);
								view.updatePawn(player1.getFirstPawn(), player1.getSecondPawn(), player2.getFirstPawn(), player2.getSecondPawn());
								c.DropCard();
								//tsekaroume an teleiwse to paixnidi
								if(checkIfGameFinished()) {
				               ...
				               if(c.ableToMove(currentPlayer.getSecondPawn(),5,currentPlayer.getFirstPawn(),2, cardsNBoard)==true) {
									c.movePawn(currentPlayer.getSecondPawn(),5,currentPlayer.getFirstPawn(),2, cardsNBoard);
									view.updatePawn(player1.getFirstPawn(), player1.getSecondPawn(), player2.getFirstPawn(), player2.getSecondPawn());
									c.DropCard();
									//tsekaroume an teleiwse to paixnidi
									if(checkIfGameFinished()) {
								 ...
								 if(c.ableToMove(currentPlayer.getSecondPawn(),4,currentPlayer.getFirstPawn(),3, cardsNBoard)==true) {
										c.movePawn(currentPlayer.getSecondPawn(),4,currentPlayer.getFirstPawn(),3, cardsNBoard);
										view.updatePawn(player1.getFirstPawn(), player1.getSecondPawn(), player2.getFirstPawn(), player2.getSecondPawn());
										c.DropCard();
										//tsekaroume an teleiwse to paixnidi
										if(checkIfGameFinished()) {
								...
								if(c.ableToMove(currentPlayer.getSecondPawn(),3,currentPlayer.getFirstPawn(),4, cardsNBoard)==true) {
									c.movePawn(currentPlayer.getSecondPawn(),3,currentPlayer.getFirstPawn(),4, cardsNBoard);
									view.updatePawn(player1.getFirstPawn(), player1.getSecondPawn(), player2.getFirstPawn(), player2.getSecondPawn());
									c.DropCard();
									//tsekaroume an teleiwse to paixnidi
									if(checkIfGameFinished()) {
								...
								if(c.ableToMove(currentPlayer.getSecondPawn(),2,currentPlayer.getFirstPawn(),5, cardsNBoard)==true) {
									c.movePawn(currentPlayer.getSecondPawn(),2,currentPlayer.getFirstPawn(),5, cardsNBoard);
									view.updatePawn(player1.getFirstPawn(), player1.getSecondPawn(), player2.getFirstPawn(), player2.getSecondPawn());
									c.DropCard();
									//tsekaroume an teleiwse to paixnidi
									if(checkIfGameFinished()) {
								...
								if(c.ableToMove(currentPlayer.getSecondPawn(),1,currentPlayer.getFirstPawn(),6, cardsNBoard)==true) {
									c.movePawn(currentPlayer.getSecondPawn(),1,currentPlayer.getFirstPawn(),6, cardsNBoard);
									view.updatePawn(player1.getFirstPawn(), player1.getSecondPawn(), player2.getFirstPawn(), player2.getSecondPawn());
									c.DropCard();
									//tsekaroume an teleiwse to paixnidi
									if(checkIfGameFinished()) {
		...
		}else if((JButton)e.getSource()==view.getPlayer2Pawns()[0]) { //prwto kitrino
				...
				if(cardsNBoard.getLatestCard() instanceof SorryCard) {
					...
					if(c.IsPlayed()==false) {
						...
						String s = (String)JOptionPane.showInputDialog(
						                    ...
						                    "");
						if(s.equals(moves[0])) {
							if(c.ableToMove(currentPlayer.getFirstPawn(),player1.getFirstPawn(), cardsNBoard)==true) {
			               ...
			               if(c.ableToMove(currentPlayer.getFirstPawn(),player1.getSecondPawn(), cardsNBoard)==true) {
		...
		}else if((JButton)e.getSource()==view.getPlayer2Pawns()[1]) { //2o kitrino
if(currentPlayer.getColor()==Color.YELLOW) {
				...
				if(cardsNBoard.getLatestCard() instanceof SorryCard) {
					...
					if(c.IsPlayed()==false) {
						...
						String s = (String)JOptionPane.showInputDialog(
						                    ...
						                    "");
						if(s.equals(moves[0])) {
							if(c.ableToMove(currentPlayer.getSecondPawn(),player1.getFirstPawn(), cardsNBoard)==true) {
			               ...
			               if(c.ableToMove(currentPlayer.getSecondPawn(),player1.getSecondPawn(), cardsNBoard)==true) {
...
/**A class which implements MouseListener. We need it 
 * for the clicks on the menu bar.
*/
