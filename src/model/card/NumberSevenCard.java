...
public class NumberSevenCard extends NumberCard{
	/** Constructor. 
	* Method to create an object of type NumberSevenCard.
	* @pre 
	* @post Creates an object of type NumberSevenCard and gives to it the number seven.
	*/
	...
	/**
	 * Moves player's pawns 7 squares front in total. If 
	 * a pawn moved to a square where a pawn of different color is located, it 
	 * sends the different color pawn to its start square. Also, if the pawn moved 
	 * to a StartSlideSquare with different color then it slides to the 
	 * EndSlideSquare of this color and sends pawns which are in this whole slide
	 * square to their start squares.
	...
	/**
	 * Moves pawn front by increasePosition. If 
	 * the pawn moved to a square where a pawn of different color is located, it 
	 * sends the different color pawn to its start square. Also, if the pawn moved 
	 * to a StartSlideSquare with different color then it slides to the 
	 * EndSlideSquare of this color and sends pawns which are in this whole slide
	 * square to their start squares.
...
//else if it isn't located on a start home or safety zone square.
	...
	if(pawn.getColor()==Color.RED) { 
		if(pawn.getPosition()<=2) {
			if(pawn.getPosition()+increasePosition<=2) {
				pawn.setPosition(pawn.getPosition()+increasePosition);
				if(table.getBoard().get(pawn.getPosition()).isPawnOn()==true) { //an uparxei pawn tha einai diaforetiko xrwma afou an itan idio de tha epestrefe false i ableToMove
	 ...
	 * increasePosition2 by calling the function ableToMove(Pawn pawn,int increasePosition,Deck table) for each pawn.This move is invalid when at least one of the pawns is 
	 * located at its start square or at least one of the squares where the pawns 
	 * would move has already a same colour pawn on it. Also pawns can't move to 
	 * opponent's home and safety zone.
	...
	public boolean ableToMove(Pawn pawn1,int increasePosition1,Pawn pawn2,int increasePosition2,Deck table) {
		...
		if(pawn1.isFinished()==true || pawn2.isFinished()==true) { //an exei ftasei home
			...
			if(table.getBoard().get(pawn1.getPosition()) instanceof StartSquare || table.getBoard().get(pawn2.getPosition()) instanceof StartSquare) { //an einai se start square
				return false;
			} 
		...
		if(possiblePosition1==-1 || possiblePosition2==-1 || (possiblePosition1==possiblePosition2 && (possiblePosition1!=65 && possiblePosition1!=71))) {
			return false;
		}
		if(ableToMove(pawn1,increasePosition1,table)==true && ableToMove(pawn2,increasePosition2,table)==true) {
			return true;
		}
		return false;
	...
	/**
	 * Checks if  pawn can move front by increasePosition .This move is invalid when the pawn is 
	 * located at its start square or the square where the pawn
	 * would move has already a same colour pawn on it. Also pawn can't move to 
	 * opponent's home and safety zone.
	 * @pre Parameters not null.
	 * @post Returns true if the move is valid,else false.
				...
				if(pawn.getPosition()>=60 && pawn.getPosition()<65) { //gia to kokkino pawn
					if((pawn.getPosition()+increasePosition)<=65) {
						if(pawn.getPosition()+increasePosition==65){
							...
							if(table.getBoard().get(pawn.getPosition()+increasePosition).isPawnOn()==true) { //an yparxei allo pioni sto safety zone tote sigoura tha einai idiou xrwmatos
								return false;
							}
							return true;
						}else {
