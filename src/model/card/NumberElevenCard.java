...
public class NumberElevenCard extends NumberCard {
	...
	/**
	 * Constructor. Method to create an object of type NumberElevenCard.
	 * 
	 * @pre
	 ...
	 * can't exchange positions if pawns are located at their home or start squares
	 * and if opponent's pawn is located at its safety zone.
	 * 
	...
	public boolean ableToMove(Pawn pawn, Pawn opponentPawn, Deck table) {
		if (table.getBoard().get(pawn.getPosition()) instanceof StartSquare
				|| table.getBoard().get(pawn.getPosition()) instanceof HomeSquare
				|| table.getBoard().get(pawn.getPosition()) instanceof SafetyZoneSquare) {
			return false;
		} else {
			if (table.getBoard().get(opponentPawn.getPosition()) instanceof StartSquare
					|| table.getBoard().get(opponentPawn.getPosition()) instanceof SafetyZoneSquare
					|| table.getBoard().get(opponentPawn.getPosition()) instanceof HomeSquare) {
				return false;
			}
			return true;
		}
