...
/**
* Moves the pawn front as much as the number of the card. If the pawn moved to a square 
 ...
 * square and also can't move to opponent's home, safety zone or to a square where
 * a pawn of the same color is.
			...
			if(pawn.getPosition()>=60 && pawn.getPosition()<65) { //gia to kokkino pawn
				if((pawn.getPosition()+number)<=65) {
					if(pawn.getPosition()+number==65){
						...
						if(table.getBoard().get(pawn.getPosition()+number).isPawnOn()==true) { //an yparxei allo pioni sto safety zone tote sigoura tha einai idiou xrwmatos
							return false;
						}
						return true;
					}else {
