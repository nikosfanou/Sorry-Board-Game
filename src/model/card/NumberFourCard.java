...
public class NumberFourCard extends NumberCard{
	...
	/** Constructor. 
	* Method to create an object of type NumberFourCard.
	* @pre 
	* @post Creates an object of type NumberFourCard and gives to it the number four.
	*/
	...
	/**
	* Reduces pawns position by 4. If the pawn moved to a square 
* where a pawn of different color is located, it sends the different color pawn to its 
* start square. Also, if the pawn moved to a StartSlideSquare with different color 
* then it slides to the EndSlideSquare of this color and sends pawns which are in 
* this whole slide square to their start squares.
...
public void movePawn(Pawn pawn, Deck table) {
	...
	if(pawn.getColor()==Color.RED) {
		if(pawn.getPosition()==28 ||pawn.getPosition()==43 ||pawn.getPosition()==58) {
			...
			pawn.setPosition(pawn.getPosition()-getNumber());
			if(table.getBoard().get(pawn.getPosition()) instanceof StartSlideSquare && table.getBoard().get(pawn.getPosition()).getColor()!=Color.RED) { //tote tha pesei sti mikri skala slide
				for(int i=0;i<=3;i++) {
					if(table.getBoard().get(pawn.getPosition()+i).isPawnOn()==true) {
						sendStart(table.getBoard().get(pawn.getPosition()+i).getPawnOn(),table);
					}
		...
		if(pawn.getPosition()==28 ||pawn.getPosition()==13 ||pawn.getPosition()==58) {
			...
			pawn.setPosition(pawn.getPosition()-getNumber());
			if(table.getBoard().get(pawn.getPosition()) instanceof StartSlideSquare && table.getBoard().get(pawn.getPosition()).getColor()!=Color.YELLOW) { //tote tha pesei sti mikri skala slide
				for(int i=0;i<=3;i++) {
					if(table.getBoard().get(pawn.getPosition()+i).isPawnOn()==true) {
						sendStart(table.getBoard().get(pawn.getPosition()+i).getPawnOn(),table);
					}
 ...
 * located at its start square and also can't move to opponent's home, safety zone
 *  or to a square where a pawn of the same colour is.
...
public boolean ableToMove(Pawn pawn, Deck table) {
	...
	if(pawn.getPosition()>=60 && pawn.getPosition()<65) { //gia to kokkino pawn
		if(pawn.getPosition()==60) {
			...
			if(table.getBoard().get(pawn.getPosition()-getNumber()).isPawnOn()==true) { //an iparxei tote einai kokkino ara false.
				return false;
			}else {
				return true;
			}
			...
			if(table.getBoard().get(pawn.getPosition()-getNumber()-57).isPawnOn()==false) {
				...
				if(table.getBoard().get(pawn.getPosition()-getNumber()-57).getPawnOn().getColor()==Color.YELLOW) {
					return true;
				}else {
					return false;
				}
	...
	if(pawn.getPosition()>=66) {
		if(pawn.getPosition()==70) {
			if(table.getBoard().get(pawn.getPosition()-getNumber()).isPawnOn()==true) { //an iparxei tote einai kitrino ara false.
				return false;
			}else {
				return true;
			}
			...
			if(table.getBoard().get(pawn.getPosition()-getNumber()-33).isPawnOn()==false) {
				...
				if(table.getBoard().get(pawn.getPosition()-getNumber()-33).getPawnOn().getColor()==Color.RED) {
					return true;
				}else {
					return false;
				}
	...
	if(pawn.getColor()==Color.RED) {
		...
		if(pawn.getPosition()==28 || pawn.getPosition()==43 || pawn.getPosition()==58) {
			...
			if(table.getBoard().get(pawn.getPosition()-getNumber()+60).isPawnOn()==false) {
				...
				if(table.getBoard().get(pawn.getPosition()-getNumber()+60).getPawnOn().getColor()==Color.RED) {
					return false;
				}else {
					return true;
				}
...
if(pawn.getPosition()==28 || pawn.getPosition()==13 || pawn.getPosition()==58) {
	...
	if(table.getBoard().get(pawn.getPosition()-getNumber()+60).isPawnOn()==false) {
		...
		if(table.getBoard().get(pawn.getPosition()-getNumber()+60).getPawnOn().getColor()==Color.YELLOW) {
			return false;
		}else {
			return true;
		}
...
public String toString() {
	return "Card's number: "+getNumber()+"\n"
	          +"Attributes: With this card player can move one of his pawns 4 squares"
	          + "back.\n"; 
}
