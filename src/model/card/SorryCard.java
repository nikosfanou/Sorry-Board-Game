...
public class SorryCard extends Card {
	...
	/**
	 * Sorry card doesn't move the pawn in a different way than exchanging the
	 * positions of the player's pawn with opponent's pawn if that can happen. So
	 * this function can't do something important for our program as it doesn't take
	 * opponent's pawn as a parameter and won't be used in the program.
	 */
	...
	/**
	 * Sorry card doesn't move the pawn in a different way than exchanging the
	 * positions of the player's pawn with opponent's pawn if that can happen. This
	 * function can't do something important for our program as it doesn't take
	 * opponent's pawn as a parameter so it will always return false and won't be
	 * used in the program.
	 * 
	 * @return False
	 */
	...
	/**
	 * Checks if player's pawn and opponent's pawn can exchange positions. They
	 * can't exchange positions if opponent's pawns are located at their home,
	 * safety zone or start squares and if players's pawn isn't located at its start
	 * square.
	 * 
	 ...
	 * @param opponentPawn Opponent's pawn which we want to check if can exchange
	 *                     its position with player's pawn.
	 * @param table        The table/ squares where the pawn is moving on.
	...
	public boolean ableToMove(Pawn pawn, Pawn opponent, Deck d) {
		if (d.getBoard().get(pawn.getPosition()) instanceof StartSquare) {
			if (d.getBoard().get(opponent.getPosition()) instanceof StartSquare
					|| d.getBoard().get(opponent.getPosition()) instanceof SafetyZoneSquare
					|| d.getBoard().get(opponent.getPosition()) instanceof HomeSquare) {
				return false;
			}
			return true;
		}
	...
	public String toString() {
		return "Sorry Card\n" + "Attributes: With this card player can"
				+ "exchange positions between one of his pawns and an " + "opponent's pawn";
	}
